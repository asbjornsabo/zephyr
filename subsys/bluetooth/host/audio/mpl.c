...
static int add_icon_object(struct mpl_mediaplayer_t *pl)
{
	...
	icon.size = setup_icon_object();
	icon.name = icon_name;
	icon.type.uuid.type = BT_UUID_TYPE_16;
	icon.type.u16.val = BT_UUID_16(icon_type)->val;
	BT_OTS_OBJ_SET_PROP_READ(icon.properties, 1);

	ret = bt_ots_add_object(bt_mcs_get_ots(), &icon);
...
static int add_current_track_segments_object(struct mpl_mediaplayer_t *pl)
{
	...
	segs.size = setup_segments_object(pl->group->track);
	segs.name = pl->group->track->title;
	segs.type.uuid.type = BT_UUID_TYPE_16;
	segs.type.u16.val = BT_UUID_16(segs_type)->val;
	BT_OTS_OBJ_SET_PROP_READ(segs.properties, 1);

	ret = bt_ots_add_object(bt_mcs_get_ots(), &segs);
	if (ret) {
...
static int add_track_object(struct mpl_track_t *track)
{
	...
	track_meta.size = setup_track_object(track);
	track_meta.name = track->title;
	track_meta.type.uuid.type = BT_UUID_TYPE_16;
	track_meta.type.u16.val = BT_UUID_16(track_type)->val;
	BT_OTS_OBJ_SET_PROP_READ(track_meta.properties, 1);

	ret = bt_ots_add_object(bt_mcs_get_ots(), &track_meta);
...
static int add_group_object(struct mpl_group_t *group)
{
	...
	group_meta.size = setup_group_object(group);
	group_meta.name = group->title;
	group_meta.type.uuid.type = BT_UUID_TYPE_16;
	group_meta.type.u16.val = BT_UUID_16(group_type)->val;
	BT_OTS_OBJ_SET_PROP_READ(group_meta.properties, 1);

	ret = bt_ots_add_object(bt_mcs_get_ots(), &group_meta);
...
static int add_parent_group_object(struct mpl_mediaplayer_t *pl)
{
	...
	group_meta.size = setup_parent_group_object(pl->group);
...
static int on_obj_created(struct ots_svc_inst_t *ots, struct bt_conn *conn,
			  uint64_t id,
...
static uint32_t on_object_send(struct ots_svc_inst_t *ots, struct bt_conn *conn,
				      ...
				      struct mpl_op_ntf_t ntf)
{
	...
	case MPL_OPC_PLAY: /* Fall-through - handle several cases identically */
	case MPL_OPC_PAUSE:
	case MPL_OPC_FAST_REWIND:
	case MPL_OPC_FAST_FORWARD:
	case MPL_OPC_STOP:
	case MPL_OPC_MOVE_RELATIVE:
	case MPL_OPC_PREV_SEGMENT:
	case MPL_OPC_NEXT_SEGMENT:
	case MPL_OPC_FIRST_SEGMENT:
	case MPL_OPC_LAST_SEGMENT:
	case MPL_OPC_GOTO_SEGMENT:
		ntf.result_code = MPL_OPC_NTF_PLAYER_INACTIVE;
		mpl_operation_cb(ntf);
		break;
	case MPL_OPC_PREV_TRACK:
		if (do_prev_track(&pl)) {
			pl.track_pos = 0;
			do_track_change_notifications(&pl);
		} else {
		...
		pl.state = MPL_MEDIA_STATE_PAUSED;
		mpl_media_state_cb(pl.state);
		ntf.result_code = MPL_OPC_NTF_SUCCESS;
		mpl_operation_cb(ntf);
		break;
	case MPL_OPC_NEXT_TRACK:
		if (do_next_track(&pl)) {
			pl.track_pos = 0;
			do_track_change_notifications(&pl);
		}
	...
	case MPL_OPC_GOTO_TRACK:
		if (operation.use_param) {
			...
			ntf.result_code = MPL_OPC_NTF_CANNOT_BE_COMPLETED;
		}
		mpl_operation_cb(ntf);
		break;
	case MPL_OPC_PREV_GROUP:
	...
	default:
		...
		ntf.result_code = MPL_OPC_NTF_NOT_SUPPORTED;
		mpl_operation_cb(ntf);
		break;
	}
}
				     ...
				     struct mpl_op_ntf_t ntf)
{
	...
	case MPL_OPC_FAST_REWIND:
		...
		pl.state = MPL_MEDIA_STATE_SEEKING;
		mpl_media_state_cb(pl.state);
				    ...
				    struct mpl_op_ntf_t ntf)
{
	...
	switch (operation.opcode) {
	case MPL_OPC_PLAY:
		pl.state = MPL_MEDIA_STATE_PLAYING;
		mpl_media_state_cb(pl.state);
		ntf.result_code = MPL_OPC_NTF_SUCCESS;
		mpl_operation_cb(ntf);
		break;
				     ...
				     struct mpl_op_ntf_t ntf)
{
	...
	switch (operation.opcode) {
	case MPL_OPC_PLAY:
		pl.seeking_speed_factor = MPL_SEEKING_SPEED_FACTOR_ZERO;
		pl.state = MPL_MEDIA_STATE_PLAYING;
		mpl_media_state_cb(pl.state);
		mpl_seeking_speed_cb(pl.seeking_speed_factor);
		ntf.result_code = MPL_OPC_NTF_SUCCESS;
	...
	case MPL_OPC_FAST_REWIND:
		...
		if (pl.seeking_speed_factor >= -(MPL_SEEKING_SPEED_FACTOR_MAX \
...
void (*operation_handlers[MPL_MEDIA_STATE_LAST])(struct mpl_op_t operation,
						 struct mpl_op_ntf_t ntf) = {
...
void mpl_operation_set(struct mpl_op_t operation)
{
	...
	if (pl.state < MPL_MEDIA_STATE_LAST) {
